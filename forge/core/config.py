...
class Settings(BaseSettings):
    ...
    llm_backend: Literal["anthropic", "openai", "ollama", "llamacpp", "heuristic"] = Field(
        default="heuristic",
        description="Configured LLM backend for FORGE conversations and reasoning.",
    )
    ...
    openai_api_key: SecretStr | None = Field(
        default=None,
        description="API key used by the OpenAI backend.",
    )
    ollama_base_url: str = Field(
        default="http://localhost:11434",
        description="Base URL used for local Ollama generation.",
    )
    ollama_model: str = Field(
        default="llama3.1:8b",
        description="Default Ollama model used for local FORGE conversation flows.",
    )
    llamacpp_base_url: str = Field(
        default="http://localhost:8080",
        description="Base URL for a llama.cpp-compatible HTTP server.",
    )
    llamacpp_model: str = Field(
        default="local-gguf",
        description="Model identifier reported when using llama.cpp.",
    )
    ...
    slack_signing_secret: SecretStr | None = Field(
        default=None,
        description="Slack signing secret used for interactive action verification.",
    )
    ...
    approval_base_url: str = Field(
        ...
        description="Base URL used when generating fallback approval links.",
    )
    ...
    checkpoint_namespace: str = Field(
        ...
        description="Redis key namespace used for persisted workflow checkpoints.",
    )
    workspace_dir_name: str = Field(
        default=".forge",
        description="Hidden project directory where indexed and session state is stored.",
    )
